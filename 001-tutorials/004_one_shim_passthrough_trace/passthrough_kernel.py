...
def my_passthrough_kernel(dev, in1_size, out_size, trace_size):
    ...
    # Input
    of_in0 = ObjectFifo(line_type, name="in0")
    of_in1 = of_in0.cons().forward(name="in1", obj_type=line_type)
    
    # Output
    of_out1 = ObjectFifo(line_type, name="out1")
    of_out0 = of_out1.cons().forward(name="out0", obj_type=line_type)
    ...
    my_worker = Worker(
        core_fn,
        [of_in1.cons(), of_out1.prod(), passthrough_fn],
        trace=enable_trace,
    )
    ...
    with rt.sequence(vector_type, vector_type, vector_type) as (a_in, b_out, _):
        ...
        rt.drain(of_out0.cons(), b_out, wait=True)
