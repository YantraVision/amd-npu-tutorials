...
#define BRIGHTNESS_VAL 128 
#define SRS_SHIFT 6


template <typename T, int N>

__attribute__((noinline)) void control_brightness(T *restrict in, T *restrict out,
...
void ctrl_brightLine(uint8_t *in, uint8_t *out, int32_t lineWidth) {

  control_brightness<uint8_t, 64>(in, out, 1, lineWidth);

}



void ctrl_brightTile(uint8_t *in, uint8_t *out, int32_t tileHeight,

                     int32_t tileWidth) {

  control_brightness<uint8_t, 64>(in, out, tileHeight, tileWidth);
