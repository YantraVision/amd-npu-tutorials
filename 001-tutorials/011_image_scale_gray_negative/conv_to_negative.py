...
def conv_to_negativeAIE2(dev, width, height):
    ...
    conv_to_negativeLineKernel = Kernel(
        "conv_to_negativeLine", "conv_to_negative.cc.o", [line_ty, line_ty, np.int32]
    )
    ...
    def conv_to_negative_fn(of_in, of_out, passThroughLine):
    ...
    worker = Worker(
        conv_to_negative_fn, [of_in.cons(), of_out.prod(), conv_to_negativeLineKernel]
    )
...
module = conv_to_negativeAIE2(dev, width, height)
print(module)
