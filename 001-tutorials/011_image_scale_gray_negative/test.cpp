...
int main(int argc, const char *argv[]) {
	...
	cv::Mat outImageGray(HEIGHT, WIDTH, CV_8UC1);
	cv::Mat outImageReference = outImageGray.clone();
	...
	for (int y = 0; y < HEIGHT; y++) {
		...
		uint8_t* outPtr = bufOut;   // grayscale output buffer

		// y loop already exists
		uint8_t* outRow = outImageGray.ptr<uint8_t>(y);

		for (int x = 0; x < WIDTH; x++) {
			outRow[x] = outPtr[x];  // one byte per pixel
		}
...
cv::imwrite(fileOut, outImageGray);
