...
def my_subblock(M, K, N, Ks, generate_access_map=True):
    tensor_ty = np.ndarray[(M, K), np.dtype[np.int32]]
    data_transform = TensorAccessPattern(
        (M, K), offset=0, sizes=[1, K, M, Ks], strides=[ 1, Ks, K, 1]
    )
...
if __name__ == "__main__":
    ...
    p.add_argument("dims", help="M K Ks", type=int, nargs=3, default=[64, 64, 2])
    p.add_argument(
        "--generate-access-map",
        action="store_true",
