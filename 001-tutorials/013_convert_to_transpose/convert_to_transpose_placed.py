...
def convert_to_transposeAIE2(dev, width, height):
    ...
    data_transform_in = TensorAccessPattern(
        (M, K), offset=0, sizes=[1, int(K/Ks), M, Ks], strides=[ 1, Ks, K, 1]
    )
    
    data_transform_out = TensorAccessPattern(
        (M, K), offset=0, sizes=[int(M/Ks), int(K/Ks), Ks, Ks], strides=[ K*Ks, Ks, K, 1]
    )
    ...
    def device_body():
        ...
        convert_to_transposeLine = external_func(
            "convert_to_transposeLine", inputs=[line_ty, line_ty, np.int32]
        )
        ...
        # Compute tile 2
        @core(ComputeTile2, "convert_to_transpose.cc.o")
        def core_body():
            for _ in range_(sys.maxsize):
                for _ in range_(height):
                    ...
                    convert_to_transposeLine(elemIn, elemOut, width)
...
with mlir_mod_ctx() as ctx:
    # print(ctx.module.operation.verify())
    convert_to_transposeAIE2(dev, width, height)
    print(ctx.module)
