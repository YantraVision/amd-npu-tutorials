...
__attribute__((noinline)) void convert_to_transpose(T *restrict in, T *restrict out,
                                               const int32_t height,
                                               const int32_t width) {

  event0();
#if 1  //Transpose or passthrough
...
void convert_to_transposeLine(uint8_t *in, uint8_t *out, int32_t lineWidth) {

  convert_to_transpose<uint8_t, 64>(in, out, 1, lineWidth);

}



void convert_to_transposeTile(uint8_t *in, uint8_t *out, int32_t tileHeight,

                     int32_t tileWidth) {

  convert_to_transpose<uint8_t, 64>(in, out, tileHeight, tileWidth);
