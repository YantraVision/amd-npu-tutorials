...
def zero_paddingAIE2(dev, width, height, Kl, Kr, Mu, Md):
    ...
    data_transform_in = TensorAccessPattern(
        (M, K), offset=0, sizes=[1, 1, 1, K*M], strides=[ 1, 1, 1, 1]
    )
    
    data_transform_out = TensorAccessPattern(
        (M, K), offset=(Mu*(K + Kr + Kl) + Kl), sizes=[1, 1, M, K], strides=[ 1, 1, K + Kr + Kl, 1]
    )
    ...
    def device_body():
        ...
        zero_paddingLine = external_func(
            "zero_paddingLine", inputs=[line_ty, line_ty, np.int32]
        )
        ...
        def core_body():
            for _ in range_(sys.maxsize):
                for _ in range_(height):
                    ...
                    zero_paddingLine(elemIn, elemOut, width)
        ...
        def sequence(inTensor, notUsed, outTensor):
            ...
            in_task = shim_dma_single_bd_task(
                of_in, 
                inTensor, 
                tap=data_transform_in,
                issue_token=True
            )
            out_task = shim_dma_single_bd_task(
                of_out,
                outTensor,
                tap=data_transform_out,
                issue_token=True
            )
...
try:
    ...
    width = 256 if (len(sys.argv) != 8) else int(sys.argv[2])
    height = 256 if (len(sys.argv) != 8) else int(sys.argv[3])
    left = 16 if (len(sys.argv) != 8) else int(sys.argv[4])
    right = 16 if (len(sys.argv) != 8) else int(sys.argv[5])
    top = 16 if (len(sys.argv) != 8) else int(sys.argv[6])
    bottom = 16 if (len(sys.argv) != 8) else int(sys.argv[7])
except ValueError:
    print("Argument has inappropriate value")
with mlir_mod_ctx() as ctx:
    zero_paddingAIE2(dev, width, height, left, right, top, bottom)
    print(ctx.module)
